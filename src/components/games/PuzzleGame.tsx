...
export default function PuzzleGame() {
  ...
  const fetchTopics = async () => {
    try {
      ...
      const puzzleItems: PuzzleItem[] = selected.map(
  ...
  const checkAnswer = async () => {
    const correctCount = items.filter(
      (item) => item.correctOrder === item.currentOrder
    ).length;
  ...
  return (
    <div className="flex flex-col gap-6 p-4">
      ...
      <p className="text-center text-gray-600">
        Drag to reorder the topics in the correct sequence
      </p>
        ...
        <AnimatePresence>
          {items.map((item, index) => (
            <motion.div
              ...
              className={`flex items-center gap-3 p-4 rounded-2xl cursor-grab active:cursor-grabbing shadow-md ${
                draggedItem === item.id
