...
export async function POST(request: Request) {
  try {
    ...
    const { game, xp, activity, multiplier = 1 } = body;

    // Support both game-specific XP (from game components) and activity-based XP
    let amount: number;
    let source: string;
    let description: string;

    if (game && typeof xp === 'number') {
      // Game components send { game: "quiz", xp: 100 }
      amount = xp;
      source = game;
      description = `${game} completion`;
    } else if (activity) {
      // Legacy: { activity: "study", multiplier: 1 }
      const XP_RATES: Record<string, number> = {
        study: 10, task: 15, game: 20, streak: 30, achievement: 50,
      };
      const baseXP = XP_RATES[activity] || 10;
      amount = Math.round(baseXP * multiplier);
      source = activity;
      description = `${activity} completion`;
    } else {
      return NextResponse.json({ error: 'Either game+xp or activity is required' }, { status: 400 });
    }

    if (amount <= 0) {
      return NextResponse.json({ data: { xpEarned: 0, totalXp: 0, levelUp: false } });
    }

    await query(
      'INSERT INTO xp_transactions (user_id, amount, source, description) VALUES ($1, $2, $3, $4)',
      [OWNER_USER_ID, amount, source, description]
    );

    const profile = await query(
      'UPDATE user_game_profile SET xp_total = xp_total + $1, xp_weekly = xp_weekly + $1, games_played = games_played + 1 WHERE user_id = $2 RETURNING xp_total, level, level_title',
